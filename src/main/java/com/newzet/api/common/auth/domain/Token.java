...
public class Token {
	...
	public static Token of(String subject, Date issuedAt, Date expiredAt) {
		if (subject == null || issuedAt == null || expiredAt == null) {
			throw new TokenBadRequestException("토큰 생성에 필요한 값이 누락되었습니다.");
		}
		return new Token(UuidConverter.convert(subject), issuedAt, expiredAt);
	}
